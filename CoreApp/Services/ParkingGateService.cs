...
public class ParkingGateService(IParkingUnitOfWork unit) : IParkingGateService
{
    public async Task<PagedResult<ParkingGateDto>> GetAllAsync(int page, int pageSize)
    {
        ...
        var dtos = paged.Items.Select(g => (ParkingGateDto)g).ToList();
        return new PagedResult<ParkingGateDto>(dtos, paged.TotalCount, paged.Page, paged.PageSize);
    }
    ...
    {
        var entity = await unit.Gates.FindByIdAsync(id) ?? throw new GateNotFoundException(id);
